<script lang="ts">
	...
	export let style = "";
	export let volume = isNaN(+localStorage?.getItem("volume"))
		? 1
		: +localStorage?.getItem("volume") ?? 1;
	export let shuffle = localStorage?.getItem("shuffle") == "true" || false;
	export let urls: string[] = [];
	export let color = "black";
	export let loop: "repeat-all" | "repeat" | "no-repeat" =
		(localStorage?.getItem("loop") as "repeat-all" | "repeat" | "no-repeat") ||
		"no-repeat";
	...
	// change audio time with the input slider
